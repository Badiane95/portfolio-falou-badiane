...
async function sendEmail(payload) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(
        `[Email] Failed to send email (${response.status} ${response.statusText})${error ? `: ${error}` : ""}`
      );
      return false;
    }
    return true;
...
function generateConfirmationEmailHTML(visitorName, subject) {
  return `
    <!DOCTYPE html>
    <html>
      <head>
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>Merci pour votre message ! \u{1F389}</h1>
          </div>
          <div class="content">
            <p>Bonjour ${visitorName},</p>
            <p>Nous avons bien re\xE7u votre message avec le sujet :</p>
            <div class="message">
              <strong>"${subject}"</strong>
            </div>
            <p>Je vous remercie de votre int\xE9r\xEAt. Je vais examiner votre message attentivement et vous r\xE9pondrai dans les plus brefs d\xE9lais.</p>
            <p>En attendant, n'h\xE9sitez pas \xE0 consulter mon portfolio pour d\xE9couvrir mes projets et comp\xE9tences.</p>
            <p>Cordialement,<br><strong>Falou Badiane</strong><br>D\xE9veloppeur Web & Automation</p>
          </div>
          <div class="footer">
            <p>&copy; 2025 Falou Badiane. Tous droits r\xE9serv\xE9s.</p>
          </div>
        </div>
      </body>
    </html>
...
function generateOwnerEmailHTML(visitorName, visitorEmail, subject, message) {
  return `
    <!DOCTYPE html>
    <html>
      <head>
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <p style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
              Vous pouvez r\xE9pondre directement \xE0 <strong>${visitorEmail}</strong>
            </p>
          </div>
          <div class="footer">
            <p>Message automatique du syst\xE8me de contact du portfolio</p>
          </div>
        </div>
      </body>
    </html>
...
var appRouter = router({
  ...
  contact: router({
    send: publicProcedure.input(z3.object({
      name: z3.string().min(2, "Le nom doit contenir au moins 2 caract\xE8res"),
      email: z3.string().email("Email invalide"),
      subject: z3.string().min(5, "Le sujet doit contenir au moins 5 caract\xE8res"),
      message: z3.string().min(10, "Le message doit contenir au moins 10 caract\xE8res")
    })).mutation(async ({ input }) => {
      try {
...
${input.message}`
        ...
        if (input.email) {
          ...
          const confirmationSent = await sendEmail({
            to: input.email,
            subject: "Confirmation de r\xE9ception - Portfolio Falou Badiane",
        ...
        throw new Error("Erreur lors de l'envoi du message. Veuillez r\xE9essayer.");
      }
    })
  })
});
